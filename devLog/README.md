...
- Summary:
  - 将误设为单字母 `x` 的过滤词恢复为“伦理”和“电影解说”，避免漏拦截与英文片名误伤。
  - 对 MacCMS 被屏蔽分类的子分类、仅带分类 ID 的影片和 AI 推荐结果实施客户端过滤；修正扩充线路后的旧测试断言。
  - 将已完成的直播亮屏与平台房间本机收藏功能纳入正式 v1.3.8；61 项单元测试和 Release 构建通过，签名与 OTA 清单校验通过。

### 2026-09-17 07:01 - 生成直播收藏功能的签名 Release 测试包

- Branch doc: `devLog/release.md`
- Summary:
  - 构建包含直播亮屏与本机收藏功能的签名 Release APK，使用现有 `1.3.7 / 10307` 版本号并命名为 `TVBox-v1.3.7-live-favorites-test.apk`。
  - 签名、包内版本和构建时平台直播服务配置已核对；未上传 GitHub，也未改动正式 `v1.3.7` OTA 清单。
  - 重新取回并核对 GitHub 正式 `TVBox-v1.3.7.apk`，与测试包分开保存；按用户要求未做设备功能测试。

### 2026-09-17 06:52 - 直播播放亮屏与平台房间本机收藏

- Branch doc: `devLog/platform-live.md`
- Summary:
  - 普通电视直播和平台直播播放画面保持亮屏，退出播放页后恢复系统原有自动锁屏行为。
  - 在“直播(5)”平台选择页加入本机“收藏”入口；斗鱼、BILIBILI 等房间卡片右下角新增可单独聚焦的空心/实心星星。
  - 收藏按平台和房间 ID 保存在设备本地；收藏列表支持进入播放、返回和按列表顺序切换房间，不依赖平台列表加载成功。
  - Debug 编译与构建、收藏逻辑定向单元测试通过；全量单元测试中 3 个影视/豆瓣用例失败（未核对历史基线），未执行设备功能测试。
...
- Branch doc: `devLog/release.md`
- Summary:
  - 发布 Media3 原生控制、手机亮度/音量手势与横竖屏视频适配。
  - 版本升级为 `1.3.5 / 10305`，OTA 清单指向 GitHub v1.3.5 Release APK。
  - 已生成签名 APK，完成 SHA-256、大小与签名校验，并在已连接 Android 手机覆盖安装验证启动。

### 2026-08-16 20:52 - 播放器智能横竖屏比例适配

- Branch doc: `devLog/home-player-ui.md`
- Summary:
  - Media3 根据真实视频宽高和像素比例判断横屏或竖屏；播放器固定采用完整显示，不拉伸、不裁切。
  - 手机播放竖屏视频时自动请求竖屏，横屏视频保持横屏；退出播放器恢复应用默认横屏。
  - 电视设备不改变方向，竖屏视频在横向屏幕居中显示并保留两侧黑边。
  - 已在 `emulator-5554` 通过正式签名 APK 验证横屏视频实播和比例显示；完整单测受当前 `ContentFilter` 关键词改动影响，有两项既有过滤测试失败。

### 2026-08-16 20:29 - 手机播放亮度与音量手势

- Branch doc: `devLog/home-player-ui.md`
- Summary:
  - 播放页左半屏纵向滑动调整当前窗口亮度，右半屏纵向滑动调整媒体音量；上滑增加、下滑减少，并用半透明提示显示实时数值。
  - 保留横向进度拖动、单击控制器、双击快进/快退或暂停，以及长按 2 倍速；原生 Media3 控件区域仍优先接收触摸。
  - 离开播放器会还原进入播放器前的窗口亮度，音量保持用户调节后的系统媒体音量。
  - 已通过单元测试与 debug/release 构建，并在 `emulator-5554` 验证亮度、音量变更和亮度还原。

### 2026-08-07 13:35 - 豆瓣热播首页与 v1.3.4

- Branch doc: `devLog/douban-hot-api.md`
- Summary:
  - 首页“全部”改为“热播”，启动默认请求豆瓣最近热门剧集；卡片点击后才用当前 MacCMS 数据源搜索真实影片详情。
  - 为豆瓣接口增加持久缓存、请求头、失败回退、分页重试和海报防盗链请求头，避免接口波动影响基础观影流程。
  - 热播卡片沿用首页海报网格尺寸与遥控焦点样式；无海报、资源未命中和接口异常均有可见状态。
  - 已在 Android 9 设备安装正式签名 APK，验证首屏、延迟搜索进入详情、返回热播页和第二页分页。

### 2026-08-07 09:50 - 豆瓣热门接口说明

- Branch doc: devLog/douban-hot-api.md
- Summary:
  - 新增 Android TVBox 使用豆瓣最近热门剧集/综艺接口的请求、分类、分页、字段映射和缓存说明。
  - 记录豆瓣海报防盗链所需的图片 Referer，以及接口失败时的回退边界。
...
- Branch doc: `devLog/home-player-ui.md`
- Summary:
  - 移除 TVBox 自定义播放器操作栏，播放/暂停、进度、上/下一集、设置和倍速均由 Media3 `PlayerView` 原生控制器提供。
  - 当前播放源的全部剧集改为 Media3 播放列表；原生上/下一集切换后会同步详情页选集焦点、历史记录和播放管家状态。
  - 保留手机手势、数字 `1/3` 切集、播放管家自动换线与半透明状态提示；控制器可见时原生控件优先接收触摸。
